...
searchParamParser =
  SearchParams
    <$> strOption
          ...
          <> short 'q'
          <> help
               "search terms, space separated including twitter search keywords etc."
          <> metavar "SEARCH"
          )
    <*> option
          auto
          (  long "search_count"
          <> help "number of statuses per page"
          <> showDefault
          <> short 'c'
          <> value 100
          <> metavar "INT"
    ...
    <*> strOption
          (  long "lang"
          <> help "language code to limit results to"
          <> value "en"
          <> showDefault
          <> metavar "LANG"
          )
...
rcPathParser = strOption
  ...
  <> help "where the credentials are stored"
  <> value ".creds"
  <> showDefault
  )
...
printStatusStream delay inputStream = inputStream .| intersperseC "," .| mapM_C
  (\x -> B.putStr x >> threadDelay delay)
